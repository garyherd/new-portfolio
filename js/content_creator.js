...
function createPortfolioItems(data) {
  data.forEach(function(item) {
    ...
    client_strong.append(client_link);
    links_website_strong.append(website_link);
    links_sourceCode_strong.append(sourceCode_link);

    client_list_item.append(client_strong);
    date_list_item.append(date_strong);
    links_list_item.append(links_website_strong, links_sourceCode_strong);

    list.append(client_list_item, date_list_item, links_list_item);

    modal_body.append(title, hr, img, paragraph, list, closeButton);
  ...
  client.getEntries({
    ...
    entries.items.forEach(function(item) {
      ...
      fields.forEach(function(field) {
        console.log(field + ': ' + item.fields[field]);
      })
    }, this);
  })
}